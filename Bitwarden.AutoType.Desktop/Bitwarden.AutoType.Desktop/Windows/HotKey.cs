...
public static class WindowsIdentifiers
{
...
public sealed class WindowsHotKey : IDisposable
{
    ...
    public WindowsHotKey(VirtualKeys key, KeyModifier keyModifiers, Action<WindowsHotKey> action)
    {
        Key = key;
    ...
    {
        if (!handled && msg.message == WindowsIdentifiers.WM_HOTKEY && msg.wParam == ID)
        {
            Action?.Invoke(this);
            handled = true;
        }
...
public class HotKey : IDisposable
{
    ...
    public VirtualKeys VKey { get; private set; }
    ...
    public HotKey(VirtualKeys k, KeyModifier keyModifiers, Action<HotKey> action, bool register = true)
    {
        VKey = k;
        KeyModifiers = keyModifiers;
        Action = action;
    ...
    public bool Register2()
    {
        uint virtualKeyCode = (uint)VKey;
        Id = (int)virtualKeyCode + (int)KeyModifiers * 0x10000;
        bool result = RegisterHotKey(IntPtr.Zero, Id, (uint)KeyModifiers, virtualKeyCode);
